...
/iMP/plot/ directory has (A) general two Python scripts that can be used to plot depth profiles and signals tracked by the model
    and (B) Python scripts used to plot the results described in the model development paper. 

(A) 
    1) caco3_profiles.py    : used to plot depth profiles of major model outputs 
    You need to specify what code you used (fortran, python or matlab), simulations names  and subfolders where results are stored. 
    ...
    2) caco3_signals.py     : used to plot signal tracking diagenesis results.  
    Here, you need to specify what code you used, simulations names and subfolders where results are stored. 
    You also need to specify whether you additionally tracked particle size or 14C age or cap-D47 (with swithces of 'size' or 'isotrack' on; see other readmes).
    These details are asked when you run the code and you just need to choose the numbers that correspond to your simulations.   
    Output is stored in the same directory as plotted data is stored. 
