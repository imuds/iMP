...
# you need specigy: 
# 1) source code with which you run simulation (code = 'fortran','python' or 'matlab')
# 2) bioturbation mode (biomode = 'fickian','nobio', 'labs' or 'turbo2')
# 3) OM degradation mode (oxic-only degradation --> oxanox = False; oxic and anoxic degradation --> oxanox = True)
# 4) Simulation name  (simname) 
# 5) folder name reflecting simulation condition (filename) 

# you may also need to change: 
# a) number of profile recording duration simulation (nt = 15, in default setting)
# b) number of sediment layers (nz = 100 in default setting)
# c) number of CaCO3 classes (nsp = 4 in default setting)
...
biomode = 'fickian'
oxanox = True
simname = 'test_dis_read_wtime'
filename = 'cc-0.12E-4_rr-0.70E+0_dep-0.35E+1'

# python example
# code = 'python'
# biomode = 'fickian'
# oxanox = True
# simname = 'test_reading_time_dis4.5'
# filename = 'cc-1.2e-05_rr-7.0e-01_dep-3.5e+00'
...
# oxanox = True
# simname = 'test_reading_time_dis4.5'
# filename = 'cc-1.2e-05_rr-0.7_dep-3.5'

# inputs (a)-(c)
nt = 15  # total number of profile recording  
nz = 100 # grid number 
nsp = 4  # caco3 species number 

if biomode == 'labs':
    if oxanox:simmode = 'oxanox_labs/'
    else:simmode= 'ox_labs/'
elif biomode == 'nobio':
    if oxanox:simmode = 'oxanox_nobio/'
    else:  simmode = 'ox_nobio/'
elif biomode == 'turbo2':
    if oxanox: simmode = 'oxanox_turbo2/'
    else: simmode = 'ox_turbo2/'
else: 
    if oxanox: simmode = 'oxanox/'
    else: simmode = 'ox/'
...
if code =='fortran': om = np.empty((nt,nz,4),dtype=np.float)  # om
...
for i in range(nt):
    ...
    label[i]=str(int((rectime[i]-rectime[4])/1e3))+' kyr'
...
ax1.legend(facecolor='None',edgecolor='None',fontsize=fs,ncol=3,loc='center'
    ,handlelength=1.7
    ,handletextpad = 0.2
    )
...
##fig.tight_layout()
fig.subplots_adjust(left=0.1,bottom=0.05,wspace=0.05,hspace=0.2, top = 0.97, right = 0.97)
...
fig = plt.figure(figsize=[i*((ny)/4.) for i in figsize]) #wide
