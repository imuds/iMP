...
3) make_input.py    : Python script used to create input files (reactime.in and imp_input.in) for all versions of codes [Fortran, Python, MATLAB]
    ...
    The above two input files used for simulations in model development paper are stored in /input/EXAMPLES/ directory,
    ...
    You are asked how you make input files: 
        a) Use default backgroud values for input parameters 
            a-1) if no, you are asked to choose the parameters you want to specify background values
        b) Use default temporal changes of input parameter values 
            b-1) if no, you are asked to enter the duration of simulation, and start and end time of signal/parameter change event
                Then, you are asked to choose parameters you want to specigy the time evolution, and 
                    to specigy time and proxy/parameter values at several points. 
            ...
            b-2) if yes, asked the type of simulation you want ('biot', 'diss', 'size', 'iso', corresponding example simulations in model development paper).
